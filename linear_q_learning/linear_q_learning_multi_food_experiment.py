"""
Linear Q-Learning Multi-Food Pattern Experiment
=================================================
Evaluates trained Linear Q-Learning agents on boards that spawn multiple food
items simultaneously, placed according to different spatial patterns.
...
FOOD_PATTERNS / EPISODES_PER_CONFIG and run:
    ...
    python linear_q_learning_multi_food_experiment.py

CSVs  → linear_q_learning/multi_food_csv/
Plots → linear_q_learning/multi_food_plots/
"""
...
# Feature vector size for each encoding (must match training configuration)
...
def _load_agent(encoding_name: str):
    path = MODELS_DIR / f"linear_q_weights_{encoding_name}.pkl"
    if not path.exists():
        return None
    ...
    agent = LinearQLearningAgent(
        action_size=4,
        feature_size=ENCODING_FEATURE_SIZES[encoding_name],
        epsilon=0.0,
        epsilon_min=0.0,
        epsilon_decay=1.0,
    )
...
def run_experiment(episodes_per_config: int = EPISODES_PER_CONFIG):
    ...
    if not all_records:
        raise RuntimeError("No data generated — check that model files exist in linear_q_learning/models/.")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Linear Q-Learning Multi-Food Pattern Experiment")
