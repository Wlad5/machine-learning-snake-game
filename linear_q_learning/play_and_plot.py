...
def load_all_weights():
    ...
    for encoding in encodings:
        weights_file = models_dir / f"linear_q_weights_{encoding}.pkl"
        weights = load_weights(weights_file)
        if weights is not None:
            all_weights[encoding] = weights
        else:
            print(f"Warning: Could not load weights for {encoding}")
...
def test_all_encodings(all_weights, num_episodes=100, show_progress=True):
    results = {}

    for encoding_name in sorted(all_weights.keys()):
        if show_progress:
        ...
        agent.weights = all_weights[encoding_name]
...
if __name__ == "__main__":
    try:
        print("\n" + "="*80)
        print("LINEAR Q-LEARNING SNAKE - TEST & PLOT GENERATOR".center(80))
        print("="*80)
        ...
        print(f"Successfully loaded {len(all_weights)} weight sets")
        print(f"  Encodings: {', '.join(sorted(all_weights.keys()))}")
        ...
        # Test all encodings
        print(f"\nTesting {len(all_weights)} encodings with {num_episodes} episodes each...")
        test_data = test_all_encodings(all_weights, num_episodes=num_episodes, show_progress=True)
