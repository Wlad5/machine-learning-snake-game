...
def load_weights(filepath: Path):
...
def evaluate_single_grid(encoding_name: str, weights, cols: int, rows: int, episodes: int) -> pd.DataFrame:
...
def run_generalization_experiment(grid_sizes, episodes_per_grid: int):
    ...
    for encoding_name in ENCODINGS:
        ...
        if weights is None:
            print(f"[WARN] Missing model for {encoding_name}: {weights_file}")
            continue

        print(f"\n=== {encoding_name.upper()} ===")
        for cols, rows in grid_sizes:
            print(f"Evaluating grid {cols}x{rows} ({episodes_per_grid} episodes)...")
            grid_df = evaluate_single_grid(encoding_name, weights, cols, rows, episodes_per_grid)
            all_episode_results.append(grid_df)

    if not all_episode_results:
        raise RuntimeError("No experiment data was generated. Check model files in linear_q_learning/models.")
...
def main():
    print("\nRunning Linear Q-learning generalization experiment...")
