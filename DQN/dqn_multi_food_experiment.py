"""
DQN Multi-Food Pattern Experiment
==================================
Evaluates trained DQN agents on boards that spawn multiple food items
...
FOOD_PATTERNS / EPISODES_PER_CONFIG and run:
    ...
    python dqn_multi_food_experiment.py

CSVs  → DQN/multi_food_csv/
Plots → DQN/multi_food_plots/
"""
...
def _build_weight_matrix(cols: int, rows: int, pattern: str) -> np.ndarray:
    ...
    if pattern == "uniform":
        ...
        for y in range(rows):
            for x in range(cols):
                d = np.sqrt((x - cx) ** 2 + (y - cy) ** 2)
                # Inverse Gaussian: high at edges, low at centre
...
def _load_agent(encoding_name: str):
    path = MODELS_DIR / f"dqn_trained_model_{encoding_name}.pkl"
    if not path.exists():
        return None
    return DQNAgent.load(str(path), epsilon=0.0)
...
def run_experiment(episodes_per_config: int = EPISODES_PER_CONFIG):
    ...
    if not all_records:
        raise RuntimeError("No data generated — check that model files exist in DQN/models/.")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="DQN Multi-Food Pattern Experiment")
