...
def load_all_models():
    ...
    for encoding in encodings:
        model_file = models_dir / f"dqn_trained_model_{encoding}.pkl"
        try:
            agent = DQNAgent.load(str(model_file), epsilon=0.0)
            models[encoding] = agent
        except FileNotFoundError:
            print(f"Warning: Could not load model for {encoding} at {model_file}")
...
def test_all_encodings(models, num_episodes=100, show_progress=True):
    results = {}

    for encoding_name in sorted(models.keys()):
        if show_progress:
        ...
        agent = models[encoding_name]

        for episode in range(num_episodes):
            episode_reward, score, steps, is_win = play_episode(env, agent)
...
if __name__ == "__main__":
    try:
        print("\n" + "="*80)
        print("DQN SNAKE - TEST & PLOT GENERATOR".center(80))
        print("="*80)

        # Load pre-trained models
        print("\nLoading pre-trained DQN models...")
        models = load_all_models()

        if not models:
            print("Error: No models loaded!")
            print("Train the models first by running dqn_training.py")
            exit(1)

        print(f"Successfully loaded {len(models)} models")
        print(f"  Encodings: {', '.join(sorted(models.keys()))}")
        ...
        # Test all encodings
        print(f"\nTesting {len(models)} encodings with {num_episodes} episodes each...")
        test_data = test_all_encodings(models, num_episodes=num_episodes, show_progress=True)
