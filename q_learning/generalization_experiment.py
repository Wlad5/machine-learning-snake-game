...
def load_q_table(filepath: Path):
    if not filepath.exists():
        return None

    with open(filepath, "rb") as file:
        q_table_dict = pickle.load(file)
...
def evaluate_single_grid(encoding_name: str, q_table, cols: int, rows: int, episodes: int) -> pd.DataFrame:
    env = Snake_Env(
        render_mode=False,
        grid_cols=cols,
        grid_rows=rows,
...
def run_generalization_experiment(grid_sizes, episodes_per_grid: int):
    ...
    for encoding_name in ENCODINGS:
        ...
        if q_table is None:
            print(f"[WARN] Missing model for {encoding_name}: {q_table_file}")
            continue

        print(f"\n=== {encoding_name.upper()} ===")
        for cols, rows in grid_sizes:
            print(f"Evaluating grid {cols}x{rows} ({episodes_per_grid} episodes)...")
            grid_df = evaluate_single_grid(encoding_name, q_table, cols, rows, episodes_per_grid)
            all_episode_results.append(grid_df)

    if not all_episode_results:
        raise RuntimeError("No experiment data was generated. Check model files in q_learning/models.")
...
def main():
    print("\nRunning Q-learning generalization experiment...")
